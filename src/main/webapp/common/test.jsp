...
<head>
	...
	*/
		$(document).ready(function(){
			//${'html'}.not(:header).addClass('backgrdoud')
		});
	</script>
</head>
<body>
	...
	<header>
				...
				<c:if test="${whologin == 1}">
					...
					<li>
						<c:if test="${whologin == 1}"><%--command고치기 --%>
							<a href="<%=noForm%>main&id=${sessionScope.loginfo.id}">장바구니<span class="glyphicon glyphicon-shopping-cart"></span></a>
						</c:if>
					</li>
					<li>
						<c:if test="${whologin == 1}">
		...
		<div class="HeadNav">
			<ul>
				<li><a class="nav" href="<%=noForm%>CCBoMainList">쿠킹 클래스</a></li>
				<li><a class="nav" href="<%=noForm%>reList">나의 레시피</a></li>
				<li><a class="nav" href="<%=noForm%>main">오늘의 식자재</a></li>
				<li><a class="nav" href="<%=noForm%>main">고객 센터</a></li>
				<%-- <li>
					<c:if test="${whologin == 2}">
						<a href="#">관리자 페이지</a>
					</c:if>
		...
		<div class="footerDiv">
			<ul>
				<li>
					...
					주식회사 쿳캣즈는 전자상거래 등에서의 소비자보호에 관한 법률에 따른
